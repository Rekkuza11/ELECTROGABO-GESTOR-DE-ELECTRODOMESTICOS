...
def abrir_clientes_empleado(parent: ctk.CTkFrame) -> None:
    ...
    _tarjeta(fila_cards, "👥", "#ec4899",
             str(total_c), "Total Clientes", "Registrados en el sistema")
    ...
    # ── Layout: izquierda lista / derecha historial ───────────────────────────
    ...
    tabla_cli = _crear_tabla(panel_izq, _COLS, anchos=_ANCHOS, altura=13, expandir=True)
    ...
    _ANCH_H  = [90, 150, 110]
    tabla_hist = _crear_tabla(panel_der, _COLS_H, anchos=_ANCH_H, altura=10, expandir=True)

    lbl_total_cli = ctk.CTkLabel(
        panel_der, text="",
...
def _tarjeta(parent, icono, color, valor, titulo, subtitulo):
    ...
    ctk.CTkLabel(card, text=subtitulo, font=("Arial", 11),
                 text_color="gray").pack(anchor="w", padx=20, pady=(0, 16))


def _crear_tabla(parent, columnas, anchos=None, altura=8, expandir=False):
    estilo = ttk.Style()
    ...
    modo = "both" if expandir else "x"
    frame.pack(fill=modo, expand=expandir, pady=(0, 8))
