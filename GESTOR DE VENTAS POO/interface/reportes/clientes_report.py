...
def abrir_reporte_clientes(parent: ctk.CTkFrame) -> None:
    ...
    _tarjeta(fila_cards, "👥", "#ec4899",
             str(total_clientes), "Total Clientes", "Registrados en el sistema")
    _tarjeta(fila_cards, "✓", "#22c55e",
             str(activos), "Clientes Activos", f"{tasa}% con compras")
    _tarjeta(fila_cards, "🆕", "#06b6d4",
             str(nuevos_mes), "Nuevos Este Mes", "Primera compra en el mes")
    _tarjeta(fila_cards, "$", "#2563eb",
             formatear_moneda(gasto_prom), "Gasto Promedio", "Por cliente activo")
    ...
    tabla_top = _crear_tabla(scroll, cols_top)
    ...
    if not sin_compras:
        ...
        for id_c, nombre, telefono in sin_compras:
            fila = ctk.CTkLabel(inactivos_frame,
                                text=f"  •  [{id_c}]  {nombre}  —  Tel: {telefono}",
                                font=("Arial", 12), text_color="#92400e",
                                fg_color="#fef3c7", corner_radius=6, anchor="w")
    ...
    tabla_all = _crear_tabla(scroll, cols_all)
    ...
    tabla_hist = ttk.Treeview(tabla_hist_frame, columns=cols_hist,
                              show="headings", style="ElectroGabo.Treeview", height=4)
    for col in cols_hist:
        tabla_hist.heading(col, text=col)
        tabla_hist.column(col, anchor="center", width=160)
 
    scroll_hist = ttk.Scrollbar(tabla_hist_frame, orient="vertical",
                                command=tabla_hist.yview)
    tabla_hist.configure(yscrollcommand=scroll_hist.set)
    tabla_hist.pack(side="left", fill="both", expand=True, padx=10, pady=8)
    scroll_hist.pack(side="right", fill="y", pady=8)
